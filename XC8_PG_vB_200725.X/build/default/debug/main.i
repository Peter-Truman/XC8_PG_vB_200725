...
void draw_menu(void) {
    ...
    for (int i = 0; i < 4; i++) {
        LCD_SetCursor((unsigned char)(i + 1), 0);
        if (i == current_index) LCD_Data('>');
        else LCD_Data(' ');
...
void draw_edit(void) {
    ...
    LCD_SetCursor(1, 0);
    LCD_String("  Value: ");
    sprintf(buf, "%d %s", temp_value, params[current_index].unit);
    LCD_String(buf);
    LCD_SetCursor(3, 0);
    ...
    while (1) {
        ...
        switch (current_state) {
            ...
            case MENU:
                if (enc) {
                    current_index += enc;
                    if (current_index < 0) current_index = 4 - 1;
